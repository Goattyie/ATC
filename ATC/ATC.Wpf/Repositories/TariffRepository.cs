...
namespace ATC.Wpf.Repositories
{
    internal class TariffRepository : AbstractRepository<Tariff>, ITariffRepository
    {
        ...
        {
            await using var cmd = new NpgsqlCommand("INSERT INTO tariffs (name) VALUES (@name)", Connection);

            cmd.Parameters.AddWithValue("name", model.Name);
            await cmd.ExecuteNonQueryAsync();

            await using var cmd2 = new NpgsqlCommand($"SELECT id FROM tariffs WHERE name = @name;", Connection);
